...
### Changed
- **Pipeline Execution Routing**: `stepbit-app` now executes pipelines against the correct `stepbit-core` pipeline endpoint instead of routing them through reasoning execution.
- **Pipeline Execution UX**: Added `RLM` toggle support and surfaced tool-call output in the execution modal.

### Fixed
- **Core Status Semantics**: Distinguished `online` from `ready` so the UI can show warming vs ready states correctly.
