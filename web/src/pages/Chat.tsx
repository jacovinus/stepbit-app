...
import { Send, Plus, Trash2, Bot, Loader2, Edit3, Settings, X, Check, Globe, Brain, ChevronDown, BookOpen, Download, Upload, AlertTriangle } from 'lucide-react';
...
import { SkillsSelector } from '../components/chat/SkillsSelector';
...
export const Chat = () => {
    ...
    const handleExport = async () => {
        ...
        try {
            const blob = await sessionsApi.export(activeSessionId);
    ...
    const handleImport = async () => {
        ...
        try {
            const newSession = await sessionsApi.import(importText);
            queryClient.invalidateQueries({ queryKey: ['sessions'] });
            setActiveSessionId(newSession.id);
    ...
    const handlePurge = async () => {
        try {
            await sessionsApi.list().then(() => {
                // Call purge via DELETE /sessions
                return fetch(`${import.meta.env.VITE_API_BASE_URL || '/api'}/sessions`, {
                    method: 'DELETE',
                    headers: { 'X-API-Key': localStorage.getItem('jacox_api_key') || 'sk-dev-key-123' }
                });
            });
            queryClient.invalidateQueries({ queryKey: ['sessions'] });
    ...
    return (
        <div className="flex h-[calc(100vh-140px)] gap-6 min-w-0 overflow-hidden">
                ...
                <div className="p-4 border-b border-gruv-dark-4/20 flex flex-col gap-2">
                    ...
                    <div className="flex gap-2">
                        <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[11px] rounded-lg border border-gruv-dark-4 text-gruv-light-4 hover:text-monokai-aqua hover:border-monokai-aqua/30 transition-colors"
                        >
                            <Upload className="w-3 h-3" /> Import
                        </button>
                        <button
                            onClick={() => setIsPurgeConfirmOpen(true)}
                            className="flex-1 flex items-center justify-center gap-1.5 py-1.5 text-[11px] rounded-lg border border-gruv-dark-4 text-gruv-light-4 hover:text-monokai-pink hover:border-monokai-pink/30 transition-colors"
                        >
                        ...
                        <div className="p-4 border-b border-gruv-dark-4/20 flex justify-between items-center bg-gruv-dark-2/30">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    ...
                                    className="p-2 hover:bg-gruv-dark-3 rounded-xl transition-colors text-gruv-light-4 hover:text-monokai-green flex items-center gap-2 text-xs"
            ...
            {isImportModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="w-full max-w-2xl glass rounded-3xl overflow-hidden shadow-2xl border border-gruv-dark-4/30">
                        <div className="p-6 border-b border-gruv-dark-4/20 flex justify-between items-center bg-gruv-dark-2/30">
                            <h3 className="font-bold flex items-center gap-2">
                                <Upload className="w-5 h-5 text-monokai-aqua" />
                                Import Session
                            </h3>
                            <button onClick={() => setIsImportModalOpen(false)} className="p-1 hover:bg-gruv-dark-4 rounded-lg transition-colors">
                                <X className="w-5 h-5" />
                            </button>
                        </div>
                        <div className="p-6 flex flex-col gap-4">
                            <p className="text-xs text-gruv-light-4">Paste the contents of an exported session file (.txt) below, or upload a file.</p>
                            <label className="flex items-center justify-center gap-2 p-3 border border-dashed border-gruv-dark-4 rounded-xl hover:border-monokai-aqua/50 cursor-pointer transition-colors text-sm text-gruv-light-4 hover:text-monokai-aqua">
                            ...
                            <textarea
                                ...
                                className="w-full h-64 bg-gruv-dark-3 border border-gruv-dark-4 rounded-2xl p-4 font-mono text-xs text-gruv-light-3 leading-relaxed focus:outline-none focus:border-monokai-aqua transition-colors resize-none"
                                placeholder={"Session: My Chat\nID: ...\nCreated At: ...\n---\n[USER]: Hello!\n---\n[ASSISTANT]: Hi there!\n---"}
                                spellCheck={false}
                            />
                        </div>
                        <div className="p-6 border-t border-gruv-dark-4/20 flex justify-end gap-3 bg-gruv-dark-2/30">
                            <button onClick={() => setIsImportModalOpen(false)} className="px-5 py-2 hover:bg-gruv-dark-4 rounded-xl transition-colors text-sm font-semibold">Cancel</button>
                            <button
            ...
            {isPurgeConfirmOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
                    <div className="w-full max-w-md glass rounded-3xl overflow-hidden shadow-2xl border border-monokai-pink/30">
                        <div className="p-8 flex flex-col items-center gap-6 text-center">
                            <div className="w-16 h-16 rounded-2xl bg-monokai-pink/10 flex items-center justify-center">
                                <AlertTriangle className="w-8 h-8 text-monokai-pink" />
                            </div>
                            <div>
                                <h3 className="text-xl font-bold text-gruv-light-1">Purge All Sessions</h3>
                                <p className="text-sm text-gruv-light-4 mt-2">
                            ...
                            <div className="flex gap-3 w-full">
                                <button
                                    ...
                                    className="flex-1 px-5 py-2.5 hover:bg-gruv-dark-4 rounded-xl transition-colors text-sm font-semibold"
                                >
                                    Cancel
                                </button>
                                <button
                                    onClick={handlePurge}
                                    className="flex-1 px-5 py-2.5 bg-monokai-pink text-white rounded-xl font-semibold text-sm hover:brightness-110 transition-all shadow-lg shadow-monokai-pink/20"
                                >
                                    Purge Everything
                                </button>
                            </div>
