    ...
    <div className="p-8 max-w-7xl mx-auto min-h-screen">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
        <div className="flex items-center gap-4">
          <div className="p-3 rounded-2xl bg-monokai-green/20 border border-monokai-green/30">
            <Clock3 className="w-8 h-8 text-monokai-green" />
          </div>
          <div>
            <h1 className="text-4xl font-black text-gruv-light-0 tracking-tight">Execution History</h1>
            <p className="text-gruv-light-4 font-medium">Local history of actions triggered from stepbit-app across pipelines, jobs, triggers, and events.</p>
          </div>
        </div>
          ...
          <div className="flex items-center gap-2 rounded-2xl border border-white/10 bg-white/5 p-2">
            {(['all', 'completed', 'failed', 'running'] as const).map((status) => (
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-bold transition-colors ${statusFilter === status ? 'bg-monokai-green text-black' : 'text-gruv-light-3 hover:bg-white/10'}`}
              >
                {status}
              </button>
            ))}
      ...
      {loading ? (
        ...
        <div className="text-center py-24 border border-dashed border-white/10 rounded-[2rem] glass">
          <Activity className="w-12 h-12 text-gruv-light-4 mx-auto mb-4" />
          <p className="text-gruv-light-3 text-lg font-semibold">No execution history yet</p>
          <p className="text-gruv-light-4 mt-2">Run a pipeline, trigger a job, create a trigger, or publish an event to populate this view.</p>
        </div>
      ) : (
        <div className="space-y-4">
            ...
            >
              <div className="space-y-3">
                <div className="flex items-center gap-3 flex-wrap">
                  <h3 className="text-xl font-bold text-gruv-light-1">#{run.id}</h3>
                  <span className="px-3 py-1 rounded-full text-xs font-bold bg-monokai-aqua/15 text-monokai-aqua border border-monokai-aqua/20">
                    {run.source_type}
                  </span>
...
const StatusBadge = ({ status }: { status: string }) => {
  if (status === 'completed') {
    return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-monokai-green/15 text-monokai-green border border-monokai-green/20"><CheckCircle2 className="w-3 h-3" />completed</span>;
  }
  if (status === 'failed') {
    return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-monokai-pink/15 text-monokai-pink border border-monokai-pink/20"><XCircle className="w-3 h-3" />failed</span>;
  }
  return <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold bg-monokai-orange/15 text-monokai-orange border border-monokai-orange/20"><Clock3 className="w-3 h-3" />running</span>;
};
